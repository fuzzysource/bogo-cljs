(ns bogo-cljs.bg-typemode
  (:require [clojure.string :as string]
            [bogo-cljs.bg-word :refer [normalize
                                       mark->word
                                       ]]
            [bogo-cljs.bg-action :refer [rollback-transformation]])
  (:require-macros [bogo-cljs.macros :refer [mark->
                                             accent->
                                             add->]]))
   ...
   "w" (fn [word]
         (let [case-a (mark->word word "breve")
           ...
           (cond
            ...
            :else (.concat (rollback-transformation word "horn") "w"))))
   "o" (mark-> "o" "hat" ["o" "ô" "ơ"])
   "e" (mark-> "e" "hat" ["e" "ê"])
   "d" (mark-> "d"
               "bar" ["d" "đ"])
